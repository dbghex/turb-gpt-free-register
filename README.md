...
在「配置 → 功能开关」开启「注册后设置登录密码」。浏览器仅完成注册；账号保存后，后台通过协议先尝试设置密码，再按「启用 2FA(TOTP)」开关执行 2FA。密码失败不会阻止 2FA，也不会使已经注册的账号重新进入邮箱池。

固定登录密码至少 12 字符；未配置时每账号生成独立的 16 位随机密码。注册过程中已有密码的账号不重复修改。成功密码沿用账号密码字段，可查看和导出。密码提交超时标记为结果未确认，不自动重发；服务重启也不自动重放中断的任务。

「账号 → 更多 → 密码日志」每 5 秒刷新后台密码阶段日志，显示本次设置密码及重认证验证码。2FA 日志保持独立。账号列表密码栏显示排队、运行、成功、失败或结果未确认状态。


### MoeMail 永久邮箱

在「配置 → 邮箱 / OTP → MoeMail」填写 API 地址和 API Key，点击「获取域名」，选择固定域名后保存。默认服务地址为 `https://edu.email.598888888.xyz`。Key 通过 `X-API-Key` 发送，保存在 `.env`，页面不回显。
