...
class RoxyBrowserClientRetryTests(unittest.TestCase):
    ...
    def test_create_explicit_preconnect_failure_retries_with_same_payload(self, sleep_mock):
        client = self._client([
            _FakeResponse({"code": 1, "msg": "Client network socket disconnected before secure TLS connection was established"}),
    ...
    @patch("core.roxybrowser_client._cfg.ROXY_CREATE_RETRY_DELAY", 3)
    @patch("core.roxybrowser_client._cfg.ROXY_CREATE_RETRIES", 3)
    def test_create_unknown_transport_timeout_is_not_replayed(self, sleep_mock):
        client = self._client([
